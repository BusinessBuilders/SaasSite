...
**llms.txt / llms-full.txt (AI assistants)** — `public/llms.txt` is the short,
hand-written index (llmstxt.org format: summary, one line per page, quick
facts, contact); update it when a price or page changes. `/llms-full.txt` is
GENERATED by `src/app/llms-full.txt/route.ts` from the same data modules the
pages render from, so it never needs editing. The footer links to llms.txt on
